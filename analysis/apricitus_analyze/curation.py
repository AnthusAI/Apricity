"""The curation feed: analyzers and agents propose slices; people judge them.
    ...
    PYTHONPATH=analysis analysis/.venv/bin/python -m apricitus_analyze.curation feed
...
class Store:
    ...
    def manifest_path(self, clip: str) -> pathlib.Path:
        return self.samples / f"{clip}.apricitus.json"
...
def find_breaks(store: Store, source_clip: str, sizes=(4, 8), min_lift_db: float = 6.0, max_breaks: int = 4) -> list[dict]:
    ...
    stems = {}
    for p in sorted((store.samples / stem_dir).glob("*.apricitus.json")):
...
def propose_all(store: Store, root: pathlib.Path) -> list[dict]:
    ...
    manifests = sorted(root.rglob("*.apricitus.json")) if root.is_dir() else [root.with_name(root.name + ".apricitus.json")]
    out = []
    for mp in manifests:
        clip = str(mp.relative_to(store.samples.resolve()))[: -len(".apricitus.json")]
...
def terminal_feed(store: Store, kind: str | None = None, audio: bool = True) -> int:
    ...
    if not feed(store, kind=kind, limit=1):
        print("The feed is empty. Propose some: python -m apricitus_analyze.curation propose samples")
        return 0
    print("keys: k keep · 1–5 keep with stars · s skip · l later · t tags · n name · c crate · r replay · q quit\n")
    player = None
    tmp = pathlib.Path(tempfile.gettempdir()) / "apricitus-audition.wav"
    seen: set[str] = set()
    i = 0
    while True:
