"""Split recordings into stems (drums, bass, other, vocals) with Demucs, and analyze each stem.
    ...
    PYTHONPATH=analysis analysis/.venv/bin/python -m apricitus_analyze.stems samples/marine-band/WashingtonPost.mp3
...
def run(audio: pathlib.Path, with_notes: bool = True, threads: int = 4) -> list[pathlib.Path]:
    ...
    audio = audio.resolve()
    parent_manifest = audio.with_name(audio.name + ".apricitus.json")
    if not parent_manifest.exists():
        raise SystemExit(f"{audio}: analyze it first (apricitus-analyze), so stems can share its beat grid")
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="apricitus-stems", description=__doc__.split("\n\n")[0])
