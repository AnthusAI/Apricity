"""Local server for the Apricitus web app.

    PYTHONPATH=analysis analysis/.venv/bin/python -m apricitus_analyze.server
...
def _summary(manifest_path: pathlib.Path, credits: dict) -> dict:
    ...
    audio_rel = str(manifest_path.relative_to(ROOT))[: -len(".apricitus.json")]
    in_samples = str(manifest_path.relative_to(SAMPLES))[: -len(".apricitus.json")]
...
def clips():
    ...
    out = sorted((_summary(p, credits) for p in SAMPLES.rglob("*.apricitus.json")), key=lambda c: (order.get(c["group"], 3), c["path"]))
    pending = [str(p.relative_to(ROOT)) for p in sorted(SAMPLES.rglob("*")) if p.suffix.lower() in AUDIO and not p.with_name(p.name + ".apricitus.json").exists()]
...
async def put_annotations(path: str, request: Request):
    ...
    mpath = audio.with_name(audio.name + ".apricitus.json")
    if not mpath.exists():
...
DIST = ROOT / "web" / "dist"
WASM = ROOT / "target" / "wasm32-wasip1" / "release" / "apricitus_web.wasm"


@app.get("/apricitus_web.wasm")
def wasm():
