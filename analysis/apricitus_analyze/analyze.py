"""Analyze one audio file into a Apricitus clip manifest (see schema/clip-manifest.schema.json)."""
...
def manifest_path(audio: pathlib.Path) -> pathlib.Path:
    return audio.with_name(audio.name + ".apricitus.json")
...
def analyze(path: pathlib.Path, with_notes: bool = True, rhythm_from: dict | None = None) -> dict:
    ...
    m = {
        ...
        "analysis": {
            ...
            "tools": {"apricitus-analyze": "0.2.0", "essentia": version("essentia"), "beat_this": "final0",
