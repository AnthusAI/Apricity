"""Backfill rhythm.beat_loudness and rhythm.loudness into existing manifests (cheap; no re-analysis).
    ...
    PYTHONPATH=analysis analysis/.venv/bin/python -m apricity_analyze.loudness samples
"""
...
def main(paths: list[str]) -> int:
    for root in map(pathlib.Path, paths):
        for mpath in sorted(root.rglob("*.apricity.json")) if root.is_dir() else [root]:
            m = json.loads(mpath.read_text())
            audio = mpath.with_name(mpath.name[: -len(".apricity.json")])
