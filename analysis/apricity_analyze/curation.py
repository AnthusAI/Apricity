...
def from_markup(store: Store, clip: str) -> list[dict]:
    ...
    for s in ann.get("clips", ann.get("slices", [])):
        if s.get("source") != "ml":
            continue
...
def _write_slice(store: Store, cand: dict, v: dict | None) -> str | None:
    ...
    with open(store.dir / ".lock", "w") as lock:
        ...
        slices = [s for s in ann.pop("slices", None) or ann.get("clips", []) if s.get("candidate") != cand["id"]]
        made = None
        if v is not None:
...
def export_apr(store: Store, name: str) -> str:
    ...
    for it in items:
        ...
        ann = m.get("annotations", {})
        sl = next((s["name"] for s in ann.get("clips", ann.get("slices", [])) if s.get("candidate") == it["id"]), None)
