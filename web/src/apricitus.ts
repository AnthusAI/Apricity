// Page-side Apricitus: compiling scores (wasm) and talking to the local server.
...
export function apricitusModule(): Promise<WebAssembly.Module> {
  wasmModule ??= WebAssembly.compileStreaming(fetch("/apricitus_web.wasm"));
  return wasmModule;
}

let compiler: Promise<Apricitus> | null = null;
function getCompiler() {
  compiler ??= apricitusModule().then(instantiate);
  return compiler;
}
...
export function manifest(audioPath: string, fresh = false): Promise<Manifest | null> {
  if (fresh || !manifestCache.has(audioPath)) {
    manifestCache.set(audioPath, fetch(`/files/${encodePath(audioPath)}.apricitus.json`).then((r) => (r.ok ? r.json() : null)));
  }
