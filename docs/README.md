# Apricitus documentation

Apricitus is a mashup machine: it makes music out of recordings. You describe a piece — tempo, key, a chord progression, which
clips play when — and Apricitus warps every clip onto one beat grid and transposes each one so that,
...
| [The Apricitus language](language.md) | you're writing a `.apr` score (the compact text format) |
...
| [Tools](tools.md) | you're running the analyzer, stems, the `apricitus` command or the web app |
...
exactly the same thing; `apricitus fmt` converts between them.
