...
> **Vocabulary update (2026-09-24):** the words below predate the move to the usual DAW vocabulary. `chop` is now `slice`, a manifest "slice" is now a saved **clip**, `bus` is now `group`/`return`, `gain` is now `volume`, `meter` is now `time`, and hits are now transients and one-shots. See `design/vocabulary.md` and `docs/`.
