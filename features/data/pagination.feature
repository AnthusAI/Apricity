Feature: Pagination
  ...
  Background:
    ...
    And these Slice records exist:
      """
      [
        {"id": "p0", "clipId": "clp-1", "name": "a", "start": 0, "end": 1, "source": "ml"},
        {"id": "p1", "clipId": "clp-1", "name": "b", "start": 1, "end": 2, "source": "ml"},
        {"id": "p2", "clipId": "clp-1", "name": "a", "start": 2, "end": 3, "source": "ml"},
        {"id": "p3", "clipId": "clp-1", "name": "b", "start": 3, "end": 4, "source": "ml"},
        {"id": "p4", "clipId": "clp-1", "name": "a", "start": 4, "end": 5, "source": "ml"}
      ]
      """

  Scenario: The limit applies before the filter
    When I query Slice by slicesByClip with:
      """
      {"key": {"clipId": "clp-1"}, "filter": {"name": {"eq": "a"}}, "limit": 2}
      """
  ...
  Scenario: A page can be empty while a next token remains
    When I query Slice by slicesByClip with:
      """
      {"key": {"clipId": "clp-1"}, "filter": {"name": {"eq": "a"}}, "limit": 1}
      """
  ...
  Scenario: Pages continue after the last key even when records are added
    ...
    Given these Slice records exist:
      """
      [{"id": "p05", "clipId": "clp-1", "name": "a", "start": 0.5, "end": 1, "source": "ml"}]
      """
    When I ask for the next page
    Then data contains exactly these items in this order:
      """
