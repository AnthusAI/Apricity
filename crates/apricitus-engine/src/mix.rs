...
pub struct BusDef {
    pub name: String,
    pub effects: Vec<apricitus_score::score::Effect>,
    /// Linear fader.
    pub gain: f32,
    pub out: String,
}
...
impl Mix {
    ...
    pub fn build(&self, controls: &HashMap<String, TrackControl>) -> (Arrangement, Vec<(String, master::Reductions)>) {
        ...
        for t in &self.tracks {
            ...
            let (pl, pr) = apricitus_dsp::fx::balance(t.pan as f64);
...
mod tests {
    use super::*;
    use apricitus_score::score::{DelaySpec, Effect, ReverbSpec, ReverbType};
    ...
    fn a_bus_ducks_under_a_voice() {
        ...
        let duck = Effect::Comp(apricitus_score::score::CompSpec {
            ratio: 4.0,
            threshold: -30.0,
            attack_ms: Some(2.0),
    ...
    fn tails_wrap_so_the_loop_is_seamless() {
        ...
        let mut rv = apricitus_dsp::space::Reverb::new(master::reverb_params(match &fx[0] { Effect::Reverb(r) => r, _ => unreachable!() }), 48_000.0);
