...
use apricitus_dsp::{stretch_offline, StretchParams, WarpMode};
use apricitus_score::compile::{Event, DEFAULT_LOUDNESS};
use apricitus_score::score::{FilterSpec, WarpModeSpec};
use apricitus_score::Timeline;
...
impl Renderer {
    ...
    pub fn arrange(&mut self, tl: &Timeline, range: Option<(f64, f64)>) -> Result<(Arrangement, ArrangeStats), String> {
        ...
        let keys_of = |name: &str| -> Vec<String> {
            info(name).map_or_else(Vec::new, |t| t.effects.iter().filter_map(|e| if let apricitus_score::score::Effect::Comp(c) = e { c.sidechain.clone() } else { None }).collect())
        };
...
pub fn render_event(e: &Event, src: &Audio, frames_per_beat: f64, out_sr: u32, out_len: usize) -> Stereo {
    ...
    if e.mode == WarpModeSpec::Off {
        ...
        let mut chans: Vec<Vec<f32>> = src.channels.iter().take(2).map(|c| apricitus_dsp::resample::varispeed(c, start, step, out_len)).collect();
