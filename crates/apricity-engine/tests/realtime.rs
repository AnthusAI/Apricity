...
use apricity_engine::mixer::XFADE;
use apricity_engine::master::{MasterParams, Stage};
use apricity_engine::{engine, Arrangement, Command, Placement, Stem, SwapAt, TrackControl};
use apricity_dsp::fx::{CompParams, EqParams, BiquadKind};
...
fn run(mixer: &mut apricity_engine::Mixer, frames: usize, block: usize) -> Vec<f32> {
...
fn live_fader_mute_and_solo() {
    ...
    ctl.send(Command::Play).unwrap();
    let level = |mix: &mut apricity_engine::Mixer| *run(mix, 64, 64).last().unwrap();
