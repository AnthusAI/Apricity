//! `apricitus play`: live playback with hot reload. Save the score and the change lands at the next
...
use apricitus_engine::{engine, Command, Controller, Mixer, Renderer, SwapAt, TrackControl};
...
fn compile(score: &Path) -> Result<Timeline, Vec<String>> {
    apricitus_score::compile_file(score)
}
