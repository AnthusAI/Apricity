...
use apricitus_engine::{Renderer, Stereo};
...
pub fn render(tl: &Timeline, beats: Option<(f64, f64)>) -> Result<Rendered, String> {
    ...
    let mix = arr.bounce();
    let lufs = apricitus_dsp::fx::loudness_lufs(&mix[0], &mix[1], OUT_SR as f64);
...
fn mix_report(r: &Renderer, arr: &apricitus_engine::Arrangement) -> Vec<String> {
    let Some(m) = r.mix() else { return Vec::new() };
    let level = |b: &Stereo| {
        let lufs = apricitus_dsp::fx::loudness_lufs(&b[0], &b[1], OUT_SR as f64);
