...
use apricitus_score::{compile, Score};
...
fn fixture(dir: &Path, name: &str, bpm: f64) -> PathBuf {
    ...
    std::fs::write(dir.join(format!("{name}.apricitus.json")), manifest.to_string()).unwrap();
    audio
}
...
fn speech_fixture(dir: &Path) {
    ...
    std::fs::write(dir.join("speech.wav.apricitus.json"), manifest.to_string()).unwrap();
}

fn tmp() -> PathBuf {
    let d = std::env::temp_dir().join(format!("apricitus-score-test-{}-{}", std::process::id(), rand_suffix()));
...
fn run(yaml: &str) -> Result<apricitus_score::Timeline, Vec<String>> {
    let dir = tmp();
...
fn hits_of(tl: &apricitus_score::Timeline, track: &str) -> Vec<(f64, f64, f64)> {
...
"#)
    ...
    assert_eq!(e.mode, apricitus_score::score::WarpModeSpec::Off);
