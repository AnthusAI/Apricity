//! The Apricitus text language (`.apr` files): a compact way to write a score. It parses into
...
const STATEMENTS: &[&str] = &["apricitus", "tempo", "meter", "key", "samples", "bars", "clip", "kit", "chords", "track", "bus", "master"];
...
pub fn parse(src: &str) -> Result<(Score, SourceMap), Vec<ParseError>> {
    ...
    for (i, raw) in src.lines().enumerate() {
        ...
        let r = (|| -> Result<(), ParseError> {
            ...
            match head.text {
                "apricitus" => score.apricitus = l.num("a format version like 0.1")?,
                "tempo" => {
...
pub fn format(s: &Score) -> String {
    ...
    if (s.apricitus - 0.1).abs() > 1e-9 {
        out += &format!("apricitus {}\n", num(s.apricitus));
    }
    out += &format!("tempo {}\n", num(s.tempo));
    if s.meter != 4 {
        out += &format!("meter {}\n", s.meter);
