//! Reading `<clip>.apricitus.json` manifests (see schema/clip-manifest.schema.json).
...
impl Clip {
    pub fn load(audio: &Path) -> Result<Self, String> {
        ...
        let mpath = audio.with_file_name(format!("{}.apricitus.json", audio.file_name().unwrap().to_string_lossy()));
        let text = std::fs::read_to_string(&mpath)
            .map_err(|_| format!("{} has no analysis yet; run `apricitus-analyze {}`", audio.display(), audio.display()))?;
    ...
    pub fn from_json(audio: &Path, manifest_json: &str) -> Result<Self, String> {
        let manifest: Manifest = serde_json::from_str(manifest_json).map_err(|e| format!("{}.apricitus.json: {e}", audio.display()))?;
